...
### UI/인터랙션 업데이트 (2025-08-26)

- 지도 타입 토글: 좌상단 '전체화면' 옆에 일반/위성/하이브리드 3-버튼 추가
- 좌표 카드: 좌하단 → 우하단으로 이동, 현재 중심/마우스 좌표 표기 + `z=..`
- 줌 컨트롤: 좌표 카드 내 +/− 버튼 추가
- 토스트: 주소/사건번호 복사 시 전체 창 중앙 상단에 2s 노출(슬라이드+페이드)
- 선택 강조: 체크된 항목 위치에 화살표형 오버레이 표시(지도 중심 이동 안 함)
- 상세 열람: 마커 클릭 시 팝업만 열리고, 팝업의 ‘상세보기’ 버튼에서만 상세 다이얼로그 오픈
- 선택 전용 보기: URL 쿼리 `selOnly=1`로 상태 반영/복원. 지역 변경 시 자동 해제 및 선택 초기화

### 데이터 정합성/필터링 (2025-08-26)

- 지도는 페이지 아이템이 아니라 클라이언트 필터가 적용된 전체 결과(`mapItems`) 기준
- ‘설정 초기화’ 시 매각기일 기본값(오늘~+1개월)을 UI/데이터에 동기화
- ‘선택 항목만 보기’ 토글 추가(상세 필터 패널 전용)
